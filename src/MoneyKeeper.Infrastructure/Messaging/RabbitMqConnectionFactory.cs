...
namespace MoneyKeeper.Infrastructure.Messaging
{
        ...
        private ResiliencePipeline<IConnection> BuildPipeline()
        {
            return new ResiliencePipelineBuilder<IConnection>()
                .AddRetry(new RetryStrategyOptions<IConnection>
                {
                    ...
                    OnRetry = args =>
                    {
                        _logger.LogWarning(args.Outcome.Exception,
                            "Не удалось создать соединение с RabbitMQ. Попытка {Attempt} из {RetryCount}. Следующая попытка через {DelaySeconds:F1}с",
                            args.AttemptNumber + 1, _settings.InitialConnectionRetryCount, args.RetryDelay.TotalSeconds);
                ...
                .AddCircuitBreaker(new CircuitBreakerStrategyOptions<IConnection>
                {
                    ...
                    {
                        _logger.LogCritical(
                            "Circuit breaker для подключений к RabbitMQ РАЗОМКНУТ Причина: {Reason}",
                    ...
                    OnHalfOpened = _ =>
                    {
                        _logger.LogInformation("Circuit breaker для подключений к RabbitMQ в состоянии HALF-OPEN — пробная попытка");
                        return ValueTask.CompletedTask;
                    }
                })
                .Build();
