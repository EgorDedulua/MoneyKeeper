    ...
    public class CategoryUpsertValidator : AbstractValidator<CategoryUpsertRequest>
    {
        public CategoryUpsertValidator() 
        {
            ...
            RuleFor(x => x.Description)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().When(x => x.Description is not null).WithMessage("Описание категории не может быть пустым").WithErrorCode(ErrorCodes.CATEGORY_DESCRIPTION_IS_EMPTY)
                .MaximumLength(8000).WithMessage("Описание категории не может быть длиннее 8000 символов").WithErrorCode(ErrorCodes.CATEGORY_DESCRIPTION_IS_TOO_LONG);
